 ...
 *
 * Расписание задаётся в `vercel.json` — сейчас `0 3 * * *`, раз в сутки:
 * тариф Hobby не даёт запускать cron чаще. При TTL брони в 40 минут это
 * означает, что брошенная бронь способна продержать вещь почти сутки, так что
 * суточный прогон — страховка, а не рабочий инструмент. Роут не привязан
 * к Vercel: его может дёргать любой внешний планировщик (например, расписание
 * в GitHub Actions) или Supabase pg_cron — важно лишь передавать заголовок
